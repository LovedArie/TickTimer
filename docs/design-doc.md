...
| (offline-and-devices A–H) | `design-addendum-offline-and-devices.md` | **v30.2 + v30.2.1** — Phase 1 and the hardening half of Phase 2 of going cross-platform: the app has to survive an unreachable server before it can live on a phone. TWO problems, deliberately separated — **offline start needs only a NAME** (enough to open `data-<user>.json`; no credential, no server change), while **staying signed in needs a CREDENTIAL**. The offline door gives away nothing, and that is the argument: `data.json` was always plain JSON in the account's own folder, so login proved identity TO THE SERVER and was never a lock on the file. Fenced anyway — offered only when the server could not be REACHED (a refusal is not an invitation), and only for an account with local data. **Device tokens** are a second, different credential rather than a persisted session token: the ordinary access/refresh split, so `AuthServer`'s "persisting tokens would be persisting open doors" still holds for the session tokens it was written about — what stopped being true is its assumption that the app logs in fresh every launch. Stored SHA-256-hashed, not PBKDF2 (you stretch what a human chose; nobody guesses 128 bits of CSPRNG), opt-in per login, revocable per device from either end. Coming back online is silent: an offline session retries once a minute and calls `enableSync` mid-session on the first acceptance. Explicitly NOT done: hardening (Phase 2), a revoke UI, or a token expiry invented before anyone has lived with one. **v30.2.1 added the hardening half of Phase 2**, so `SERVER.md`'s own "do not expose it to the public internet" stops being true rather than being ignored: the bind address defaults to **127.0.0.1** (it was every interface — one forgotten flag between a hand-rolled parser and the open internet, so the risky choice is now the one you type), registration can be **invite-gated** and is checked before the account store is touched so a wrong code leaks nothing, and a **login brake** gives one address five failures per five minutes — done in the server rather than in Caddy because stock Caddy has no rate-limit directive, and making the safe deployment depend on compiling your own web server is how the safe deployment does not happen. `X-Forwarded-For` is honoured only from a loopback peer, or anyone could mint a fresh identity per attempt. CORS preflight answered (the WASM build needs it, and a browser preflights on the HEADERS, not only on the address). The gate needed a key, so the client grew an invite field and two outcomes — `TooManyAttempts` deliberately NOT phrased as "wrong password", since the brake may have caught a correct one. Templates ship as `deploy/Caddyfile.example` and `deploy/ticktimer.service.example` |
| (responsive 3.41–3.46) | `design-addendum-responsive.md` | **v30.5** — the phone stopped clipping. Not a sizing bug but a MINIMUM-size one: `MainWindow`'s compact minimum was 522x706 against a 360x800 screen, and Qt honours a minimum it cannot fit by letting the surplus hang off the edge — no scrollbar, no error. One unwrappable `QCheckBox` label on PomodoroPage (476px) clipped the *Planner*, because a `QStackedWidget`'s minimum is the max over all its pages. The replacement for `isCompactScreen()` is **container-driven**: `responsive::modeFor(width)` (pure, zero Qt includes, pinned in the Core-only suite) fed by a `ResponsiveWatcher` on the PAGE STACK — not the window, because toggling the 190px rail changes the page's width and produces no window resize at all. The mode travels as a custom `QEvent` rather than a base class or a signal, since half this codebase's widgets deliberately have no `Q_OBJECT`. Budget mandatory, mode optional — the separation that keeps the ongoing cost bounded. Two rules only the device could teach: a top-level window is clamped UP to its minimum and **never back down** (571px on a 360px screen, long after the minimum fell to 386), and a mode ladder must be **descendable** or a class is unreachable (a 236px tagline hidden only at Compact kept Medium's floor above Compact's threshold). The gate that keeps it fixed needed `QT_QPA_FONTDIR` first: offscreen ships no fonts and measured the same window at 1051px against a true 522, so the one platform CI could measure on was the one whose numbers were meaningless. Still open, and load-bearing rather than polish: the rail is in-flow, so opening it re-clips |
