...
## 3e. Phones in hand, and the schedules arc *(v30.4.1 – v31.1.0)*

*Six arcs shipped after §3d's last row without one of their own — recorded
2026-09-11 from the commits and addenda, not from memory.*

| Arc | Versions | Outcome |
|---|---|---|
| **The web build, run for real** | v30.4.1–v30.4.6 | ✅ Stage 0 closed: the WASM app loads, remembers across a reload, starts offline and reconnects silently. **v30.4.5** is the one to remember — offline work was never marked dirty, so v30.4.4's reconnect fix would have taken the *Pull* row of `syncplan::decide` and silently overwritten the work it was written to rescue; `markDirty()` first turns it into a Conflict. **v30.4.6**: `AccountStore`/`DeviceStore` moved to `QSaveFile`, found while writing the backup script. The VPS went live with a nightly backup proven by a restore. Records: `design-addendum-web.md`, `docs/ROLLOUT.md` |
| **The phone, fitted** | v30.5 | ✅ Not a sizing bug but a MINIMUM — 522px against a 360px screen, set by one unwrappable checkbox label. Container-driven `responsive::modeFor` + `ResponsiveWatcher` on the page stack; window minimum 522 → 356. The first Android compile forced `namespace sync` → `syncplan`. Record: `design-addendum-responsive.md` |
| **The phone, shaped** | v30.6–v30.8 | ✅ A mobile shell (bottom bar, painted icons, capture FAB) — the DEVICE picks the shell, the CONTAINER arranges inside it. Notifications that survive a frozen process: `Alarms.h` derives a schedule of finished alarms for `AlarmManager`, because C++ cannot run at fire time on Android. A 48dp touch-target gate whose first run named 33 offenders. Width budgets re-asked as `horizontalScrollBar()->maximum()` against seeded content, because a budget measured against an empty page certifies nothing. Records: `design-addendum-notifications.md`, `-responsive.md` |
| **The desktop, un-phoned** | v30.8.1 | ✅ Four ungated phone decisions had changed the Windows app. The rule that settles it: layout is a question about SPACE (`responsive::Mode`); gestures and hit targets are questions about the INPUT DEVICE (`isCompactScreen()`). Also `ProbeOverlay`, and release step 6 became a check (`publish-version.bat`). Records: `-responsive.md` §3.63–3.64, `design-addendum-deployment.md` |
| **Artefacts reach their homes** | *(no bump — 31 Aug)* | ✅ `/app/` had served v30.4.2 for ten days while `/version` said 30.8.1. Measuring the wire found three defects the Caddyfile hid: icons 403 from `scp -r` modes, the wasm served uncompressed at 24 MB, every `Cache-Control` rule dead from directive order. `/download/` served an APK one version behind a GitHub release that already held the right file |
| **Schedules, overlap, folders, ordering — and the format floor** | built as v31.0–v31.3.1, **released as 31.1.0** | ✅ Recurrence as its own concept, materialised as real Events; sync merges three-way against a stored base before it asks; up to three blocks in a slot; retiring a semester; order as data. Then a v30.8.1 Start Menu copy stripped a format-16 planner (5 schedules, 73 links) and the **format floor** followed: `applyJsonObject` refuses a document above `kFormatVersion`, `load()` returns a four-state `LoadResult`, the store latches read-only. **Forward-only** — it protects nothing already installed, which is what backlog B7 and H5 are about. Records: `design-addendum-schedules.md`, `-overlapping-blocks.md`, `-folder-archiving.md`, `-manual-ordering.md`, `-format-floor.md` |

**On the version labels in that last row.** The addenda number their slices
v31.0–v31.3.1 as they were built in the working tree; what shipped was one
...
| **H5 — every copy past the format floor** | ✅ Windows and the Galaxy S21 upgraded in place (same key, planner kept); `/app/` and `/download/` redeployed on release day and read back off the wire — hashes matching the builds, wasm served compressed. `/app/` had been at v30.8.1 since 31 Aug |
