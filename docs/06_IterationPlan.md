...
| **Tool use — the write boundary** | ✅ **Slice 1 shipped v29.0.0** (the machine, model-less) · **Slice 2 shipped v29.1.0** (the intake interview; format stays v13 throughout) | Where the secretary becomes real: proposal → guarded doors → **your tap** → effect. Opens with **intake** (purely additive, self-confirmed, tiny blast radius), then rescheduling. No destructive verbs — archive, never delete. Likely the point where the `Dialect` enum finally earns strategy objects (provider addendum §C) |
| **Memory** | ⬜ (v30) | The residue file: only what the app *cannot* derive, **updated not appended**, written through v29's confirm loop — one mutation system, not two |
